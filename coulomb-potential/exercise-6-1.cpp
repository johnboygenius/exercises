/**
 * @file exercise-6-1.cpp
 * @author John Davis
 * @brief Numerical mesh integration of Carbon coulomb interaction energy
 * @date 2021-12-11
 */
        ...
        for (int j = 0; j < mesh; j++)
        {
            for (int k = 0; k < mesh; k++)
            {
                ...
                if (r0 != 0.0) // neglect divergences
                {
                    ...
                    sum -= 4.0*pow(R20_0*s,2.0)/r0;
                    sum -= 4.0*pow(R21_0*px0,2.0)/r0;
                    sum -= 4.0*pow(R21_0*py0,2.0)/r0;
                    sum -= 4.0*pow(R21_0*pz0,2.0)/r0;
