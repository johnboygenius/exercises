        ...
        for (int j = 0; j < mesh; j++)
        {
            for (int k = 0; k < mesh; k++)
            {
                ...
                if (r0 != 0.0) // neglect divergences
                {
                    ...
                    double a0 = (R20_0*s -sqrt(2.0)*R21_0*py0)/sqrt(3.0);
                    double b0 = (R20_0*s + sqrt(1.5)*R21_0*px0 + sqrt(0.5)*R21_0*py0)/sqrt(3.0);
                    double c0 = (-R20_0*s - sqrt(1.5)*R21_0*px0 + sqrt(0.5)*R21_0*py0)/sqrt(3.0);
                    double d0 = R21_0*pz0;
                        ...
                        for (int jj = 0; jj < mesh; jj++)
                        {
                            for (int kk = 0; kk < mesh; kk++)
                            {
                                ...
                                if (r1 != 0.0) // neglect divergences
                                {
                                    ...
                                    double a1 = (R20_1*s -sqrt(2.0)*R21_1*py1)/sqrt(3.0);
                                    double b1 = (R20_1*s + sqrt(1.5)*R21_1*px1 + sqrt(0.5)*R21_1*py1)/sqrt(3.0);
                                    double c1 = (-R20_1*s - sqrt(1.5)*R21_1*px1 + sqrt(0.5)*R21_1*py1)/sqrt(3.0);
                                    double d1 = R21_1*pz1;
