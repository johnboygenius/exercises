/**
 * @file exercise-8-2.cpp
 * @author John Davis
 * @brief Numerical Monte Carlo integration of Carbon coulomb interaction energy (sp3, O(n) vs O(n^6))
 * @date 2021-12-24
 */
...
double calculateE(long long int iter)
{
    ...
    for (int i = 0; i < iter; i++)
    {
        ...
        if (r0 != 0.0) // neglect divergences
        {
            ...
            double a0 = 0.5*(R20_0*s + R21_0*px0 + R21_0*py0 + R21_0*pz0);
            double b0 = 0.5*(R20_0*s - R21_0*px0 - R21_0*py0 + R21_0*pz0);
            double c0 = 0.5*(R20_0*s - R21_0*px0 + R21_0*py0 - R21_0*pz0);
            double d0 = 0.5*(R20_0*s + R21_0*px0 - R21_0*py0 - R21_0*pz0);
            ...
            if (r1 != 0.0) // neglect divergences
            {
                ...
                double a1 = 0.5*(R20_1*s + R21_1*px1 + R21_1*py1 + R21_1*pz1);
                double b1 = 0.5*(R20_1*s - R21_1*px1 - R21_1*py1 + R21_1*pz1);
                double c1 = 0.5*(R20_1*s - R21_1*px1 + R21_1*py1 - R21_1*pz1);
                double d1 = 0.5*(R20_1*s + R21_1*px1 - R21_1*py1 - R21_1*pz1);
